            ...
            if (node.isVisible != true) {
              ...
              var iter = function (ala) {
                ala.forEach(function (ala) {
                  if (
                    !ala.alavalintaryhmat ||
                    ala.alavalintaryhmat.length < 4
                  ) {
                    ala.isVisible = true
                  ...
                  organisations.forEach(function (org) {
                    if (item.organisaatiot.length > 0) {
                      item.organisaatiot.forEach(function (org2) {
                        if (
                          org2.parentOidPath !== null &&
                          org2.parentOidPath.indexOf(org) > -1
                        ) {
                          item.access = true
                        }
                      })
    ...
    function ($scope, $q, _, Treemodel, HakukohdeSiirra, HakuModel, UserModel) {
      ...
      $scope.showNode = function (node) {
        if (node.tyyppi === 'VALINTARYHMA') {
          return !(
            !node.sisaltaaHakukohteita &&
            $scope.domain.search.vainHakukohteitaSisaltavatRyhmat
          ) // && $scope.userOrganizationsModel.isKKUser;
        } else {
          return !(
            !node.sisaltaaHakukohteita &&
            $scope.domain.search.vainHakukohteitaSisaltavatRyhmat
