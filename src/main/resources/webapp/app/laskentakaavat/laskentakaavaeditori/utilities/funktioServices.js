  ...
  .service('FunktioService', [
    ...
    function (FunktioKuvausResource, $log, _, $q, FunktiokuvausService) {
      ...
      this.isPainotettukeskiarvo = function (funktiokutsu) {
        ...
        if (!api.isFunktiokutsu(funktiokutsu)) {
          return false
        }
        return FunktiokuvausService.isPainotettukeskiarvoByFunktioNimi(
          api.getFunktionimi(funktiokutsu)
        )
      }

      this.isEmptyNimettyFunktioargumentti = function (
        parent,
        ...
        if (api.isRootFunktiokutsu(parent)) {
          return api.isNimettyFunktioargumentti(parent) &&
            _.isEmpty(parent.funktioargumentit[funktioargumenttiIndex])
            ? true
            : false
        } else {
          return api.isNimettyFunktioargumentti(parent) &&
            _.isEmpty(parent.lapsi.funktioargumentit[funktioargumenttiIndex])
            ? true
            : false
        }
      }
      ...
      this.isLukuarvoFunktioSlot = function (parent, funktioargumenttiIndex) {
        ...
        if (api.isRootFunktiokutsu(parent)) {
          funktiokuvaus = FunktiokuvausService.getFunktiokuvaus(
            parent.funktionimi
          )
          tyyppi = isOnNimettySlot
            ? funktiokuvaus.funktioargumentit[funktioargumenttiIndex].tyyppi
            : funktiokuvaus.funktioargumentit[0].tyyppi
          return tyyppi === 'LUKUARVOFUNKTIO'
        } else {
          funktiokuvaus = FunktiokuvausService.getFunktiokuvaus(
            parent.lapsi.funktionimi
          )
          tyyppi = isOnNimettySlot
            ? funktiokuvaus.funktioargumentit[funktioargumenttiIndex].tyyppi
            : funktiokuvaus.funktioargumentit[0].tyyppi
      ...
      this.getDefinedFunktioargumenttiCount = function (funktiokutsu) {
        if (funktiokutsu.lapsi) {
          return _.filter(funktiokutsu.lapsi.funktioargumentit, function (
            item
          ) {
          ...
          _.forEach(funktioargumentit, function (item) {
            if (!_.isEmpty(item)) {
              ...
              if (isRootFunktiokutsu === true) {
                item.funktioargumentit = api.cleanLaskentakaavaPKObjects(
                  item.funktioargumentit
                )
              } else {
                if (item && item.lapsi) {
                  item.lapsi.funktioargumentit = api.cleanLaskentakaavaPKObjects(
                    item.lapsi.funktioargumentit
                  )
                }
              }
            }
      ...
      this.getFunktiokutsuTyyppi = function (funktiokutsu) {
        return api.isRootFunktiokutsu(funktiokutsu)
          ? funktiokutsu.tyyppi
          : funktiokutsu.lapsi.tyyppi
      }
          ...
          _.forEach(funktioargumentit, function (item) {
            if (item.lapsi) {
              ...
              if (item.lapsi.funktionimi === 'PAINOTETTUKESKIARVO') {
                item.lapsi.funktioargumentit = api.addPainotettukeskiarvoParametrit(
                  item.lapsi.funktioargumentit
                )
              }
            }
      ...
      this.addMissingSyoteparametrit = function (funktiokutsu) {
        ...
        api.addSyoteparametrit(
          funktiokuvaus,
          funktiokutsu.lapsi.syoteparametrit
        )
      ...
      this.addSyoteparametrit = function (funktiokuvaus, syoteparametrit) {
        if (
          funktiokuvaus &&
          funktiokuvaus.syoteparametrit &&
          funktiokuvaus.syoteparametrit.length > syoteparametrit.length
        ) {
          _.forEach(funktiokuvaus.syoteparametrit, function (
            kuvausSyoteparametri
          ) {
            var found = _.some(syoteparametrit, function (
              funktioSyoteparametri
            ) {
              return kuvausSyoteparametri.avain === funktioSyoteparametri.avain
            })

            if (!found) {
              syoteparametrit.push({
                avain: kuvausSyoteparametri.avain,
                arvo: '',
              })
            }
          })
      ...
      this.hasFunktioargumentit = function (funktiokutsu) {
        if (_.isEmpty(funktiokutsu)) {
          throw new Error(
            'Missing parameter for Funktioservice.hasFunktioargumentit',
            arguments
          )
        }
      ...
      this.getCurrentFunktiokutsu = function (parentFunktiokutsu, childIndex) {
        return api.isRootFunktiokutsu(parentFunktiokutsu)
          ? parentFunktiokutsu.funktioargumentit[childIndex]
          : parentFunktiokutsu.lapsi.funktioargumentit[childIndex]
      }
    },
  ])

  .factory('Valintaperusteviitetyypit', [
    function () {
      'use strict'
      ...
      return [
        ...
        {
          key: 'HAKUKOHTEEN_SYOTETTAVA_ARVO',
          text: 'Hakukohteen syötettävä arvo',
        },
      ]
    },
  ])

  .factory('Arvokonvertterikuvauskielet', [
    function () {
      'use strict'
